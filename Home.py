...
def home():
    with st.sidebar:
        ...
        st.session_state["esp_base_url"] = st.text_input(
            ...
            help="Enter the IP address of your ESP32/Pi. Leave blank for demo data.",
        )
        if connected:
    ...
    c1.metric("Soil Moisture", f"{reading.get('soil_moisture', 'N/A')}%")
    c2.metric("Humidity", f"{reading.get('humidity', 'N/A')}%")
    c3.metric("Temperature", f"{reading.get('temperature', 'N/A')}°C")
    c4.metric("Data Source", "Live" if reading.get("source") == "device" else "Demo")
    ...
    feature_card(row1[1], "🌡️", "Sensor Dashboard", "Live soil moisture, humidity, temperature, and light readings.", "pages/2_Sensor_Dashboard.py", "Live", "chip-green")
    feature_card(row1[2], "🚨", "Flood / Drought Alerts", "Early warnings from soil conditions and rainfall forecast.", "pages/3_Flood_Drought_Alerts.py", "Alerts", "chip-pink")
